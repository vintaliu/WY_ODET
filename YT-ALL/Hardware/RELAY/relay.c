...
void RELAY_Control(u8 c){ //继电器的控制程序（c=0关2个继电器，c=1开继电器1，c=2开继电器2,c=3开2个继电器）
	GPIO_WriteBit(RELAYPORT,RELAY1,(BitAction)(c&0x01));//通过参数值写入接口
	GPIO_WriteBit(RELAYPORT,RELAY2,(BitAction)(c&0x02));//通过参数值写入接口
}
