...
unsigned char Uart1RecvFlag = 0;
unsigned char Uart1RXCnt = 0;

unsigned char ucEnableSoftDogFlg = FALSE;
unsigned char ucEepromBuf[66];    //保存60字节的数据
...
{
        unsigned int uiCont = 10000;
        uiCont = 10000;
        /* Loop until the end of transmission */
	      UART0->BR = (senddata & (uint16_t)0x01FF);
...
//volatile unsigned char code cucOneByteData _at_ EEPRO_One_Byte_ADDRESS;//定义一个 数据在FLASE中

//volatile unsigned char code cucBatteryType _at_ WriteBatteryTypeFlag_Addr;//铅酸还是锂电池标志位
...
void vSendOneBytes(void)//向主控发送一个Bytes
{
    ...
    //从EEPROM中读取速度
    __set_PRIMASK(1);//关闭中断//clr_EA;	//关闭所有中断，否则总是进入串口中断
    Uart1SendBuf[ucSendCont++] = ucReadEepromOneData(EEPRO_One_Byte_ADDRESS);//cucOneByteData;
    __set_PRIMASK(0);//开启中断。//set_EA; //开中断
    Uart1SendBuf[ucSendCont++] = SendTail1;
    Uart1SendBuf[ucSendCont++] = SendTail2;
    Uart1SendBuf[ucSendCont++] = SendTail3;
    Uart1SendBuf[ucSendCont++] = SendTail4;
...
void vSend60Bytes(void)
{
    ...
    {
        Uart1SendBuf[ucSendCont++] = ucEepromBuf[i];//Data_Flash_FactData[i]; //这里的数据直接对应 FALSE中的数据
    }
...
void USART1_DataProcess(void)
{
    ...
    if(Uart1RecvFlag)
    {
        ...
        case Write60Bytes:  //写入60字节的数据——写入显示板
					  vSendFeedBack();
            UART_Cmd(UART0, DISABLE);//clr_ES;//还没有调试
            __set_PRIMASK(1);//关闭中断//clr_EA;
            ...
            UART_Cmd(UART0, ENABLE);//set_ES;//还没有调试//再次开启 串口中断
            __set_PRIMASK(0);//开启中断。//set_EA;
            break;
        ...
        case OrderBatteryType:  //写入1字节的数据——写入显示板
            //            //clr_ES;//还没有调试
            //            __set_PRIMASK(1);//关闭中断//clr_EA;
            //            ucDataBase = ucBattType;
            //            if(ucBattType !=  Uart1RecvBuf[5])
            //            {
            //                EEPROM_WriteByte(WriteBatteryTypeFlag_Addr, Uart1RecvBuf[5]);
            //            }
            //            __set_PRIMASK(0);//开启中断。//set_EA;
            //            //set_ES;//还没有调试//再次开启 串口中断
            break;
