...
/**
  * @brief  Initializes the PWMx channelx Capture
  * @param  PWMx: where x can be 0, 1 to select the PWM peripheral.
  * @note
...
void PWM_CaptureInit(CMSDK_PWM_TypeDef* PWMx, PWM_CaptureInitTypeDef* CapInit)
{
	...
	wTmpReg = PWMx->CCR;
	wTmpReg &= ~(0x0F << ch * 4);
	wTmpReg |=  (((CapInit->CaptureMode) << ch * 4) | ((CapInit->Capture_Interrupt) << ch*4));
	PWMx->CCR = wTmpReg;
...
void PWM_IntConfig(CMSDK_PWM_TypeDef* PWMx, uint32_t PWM_INT, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_PWM_ALL_PERIPH(PWMx));
  assert_param(IS_PWM_CAP_INT(PWM_INT));		
  assert_param(IS_FUNCTIONAL_STATE(NewState));
