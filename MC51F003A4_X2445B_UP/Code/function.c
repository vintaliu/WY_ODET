...
void vFirstStart(void)
{
    ...
    delay(10);
    clr_EA;
    GLOBAL_SPEED_GRAD = EEPROM_Byte_Read(SPEED_ADDRESS);;//读取FLASE 中的数据内容
    ucSpeedGrad = GLOBAL_SPEED_GRAD;
    EEPROM_Read(EEPROM_START_ADDRESS_FACT_PARE, ucEepromBuf, EEPROM_START_ADDRESS_FACT_PARE_Legth);
    set_EA;
    ucBatType = EEPROM_Byte_Read(WriteBatteryTypeFlag_Addr);;//读取FLASE 中的数据内容
    if(0 == ucSpeedGrad)
    {
        ucSpeedGrad = 1;
    }
...
//
//    SET_IAPEN();
//    SET_IAPEN(); //打开FALSH IAP功能，连续写两遍有效
//    SET_EPAGEERS();
//    SET_EPAGEERS(); //页擦模式
//    IAPKEY = 0x5a;
//    IAPKEY = 0xa5;
//    SET_IAPSTR(); //IAPSTR位置一，启动IAP操作
...
//    IAPKEY = 0x5a;
//    IAPKEY = 0xa5;
//    SET_IAPTRIG0(); //IAPTRIG0置一
//    IAPKEY = 0x5a;
//    IAPKEY = 0xa5;
//    SET_IAPTRIG1(); //IAPTRIG1置一
//    IAPKEY = 0x5a;
//    IAPKEY = 0xa5;
//    SET_IAPTRIG2(); //IAPTRIG1置一
//    while (IAPCR & 0x80);
...
void vTestShowLed(unsigned int value)
{
    ...
    if (value & 0x0080) SPEED_STEP3_ON;//方便不开机的时候  快速区分按键版本的不同
...
;  *    @Description         : eeprom字擦除模式初始化
;  *    @IN_Parameter      	 :addar 0-0x3ff W_data 数据
;  *    @Return parameter    :
;  ***********************************************/
